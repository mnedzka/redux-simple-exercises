  ...
  render() {
    return (
      <div>
        ...
        />
        <hr />
        <button onClick={result => this.props.onStoreResult(this.props.ctr)}>
          Store result
        </button>
        <ul>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onSubtractCounter: () =>
      dispatch({ type: actionTypes.SUBTRACT, payload: -10 }),
    onStoreResult: result =>
      dispatch({ type: actionTypes.STORE_RESULT, result }),
    onDeleteResult: id =>
      dispatch({ type: actionTypes.DELETE_RESULT, toDelete: id })
  };
};
